...
trait ProtocolRecorder[A] {
  def withBehavior(pf: PartialFunction[Message.BestEffortByName[A], Message[A]]): UIO[ProtocolRecorder[A]]
  def collectN[B](n: Long)(pr: PartialFunction[Message[A], B]): UIO[List[B]]
  def send(msg: Message.BestEffortByName[A]): IO[zio.memberlist.Error, Message[A]]
}

object ProtocolRecorder {

  def apply[A: Tag](
    pf: PartialFunction[Message.BestEffortByName[A], Message[A]] = PartialFunction.empty
  ...
  def make[R, E, A: Tag](
    ...
    (for {
      behaviorRef  <- Ref.make[PartialFunction[Message.BestEffortByName[A], Message[A]]](PartialFunction.empty)
      ...
      override def withBehavior(
        pf: PartialFunction[Message.BestEffortByName[A], Message[A]]
      ): UIO[ProtocolRecorder[A]] =
      ...
      override def send(msg: Message.BestEffortByName[A]): IO[zio.memberlist.Error, Message[A]] =
  ...
  private def consumeMessages[A](
    ...
    message: Message[A],
    behaviorRef: Ref[PartialFunction[Message.BestEffortByName[A], Message[A]]],
    protocol: Protocol[A]
    ...
    message match {
      ...
      case md: Message.BestEffortByName[A @unchecked]    =>
        messageQueue.offer(md) *>
          behaviorRef.get.flatMap { fn =>
            ZIO.whenCase(fn.lift(md)) { case Some(d: Message.BestEffortByName[A @unchecked]) =>
