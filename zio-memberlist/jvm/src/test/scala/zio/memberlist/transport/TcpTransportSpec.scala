...
object TcpTransportSpec extends KeeperSpec {
  ...
  val makeAddr: ZIO[Live, Nothing, NodeAddress] = findAvailableTCPPort(49152, 65535).flatMap(NodeAddress.local)

  val spec: Spec[TestEnvironment, TestFailure[TransportError], TestSuccess] = (suite("TcpTransport")(
  ...
  ) @@ timeout(15.seconds) @@ flaky @@ ignore).provideCustomLayer(environment)
}
